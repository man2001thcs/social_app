	...
	*/
	...
	function resultSet(&$results) {
		...
		while ($j < $num_fields) {
			...
			if (!empty($column->table)) {
				...
				$this->map[$index++] = array("COUNT", $column->name);
			}
			$j++;
		}
	}
	...
	protected function buildConditions($condition) {
		...
		foreach ( $condition as $field => $sub ) {
			...
			} else if ((strpos($sub, "IN") !== false) || (strpos($sub, "NOT IN") !== false) 
				|| (strpos($sub, "is null") !== false) || (strpos($sub, "NOT LIKE") !== false)) {
				$conditionArray[] = " " . $field . " " . $sub . "";
			}
			else {
				//echo "this_lad";
	...
	protected function buildConditions_or($condition_or) {
		...
		foreach ( $condition_or as $conditions => $condition) {
			...
			foreach ( $condition as $field => $sub ) {
				...
				|| (strpos($sub, "NOT IN") !== false)  || (strpos($sub, "NOT LIKE") !== false)) {
					$conditionArray[] = " " . $field . " " . $sub . "";
				}
				else {
					$conditionArray[] = " " . $field . " " . $math . " '" . $sub . "'";
